...
const CatPage = () => {
  ...
  const priorityConfig = {
    urgente: {
      ...
      description: "Este gatito necesita un hogar urgentemente",
    },
    alta: {
      label: "🟡 Prioridad Alta",
      color: "bg-yellow-100 text-yellow-700 border-yellow-300",
      description: "Este gatito necesita un hogar pronto",
    },
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
              ...
              >
                <div className="flex items-start gap-16">
                  ...
                  {cat?.priority !== "normal" && (
                    <div
                      className={`text-center flex-1 mb-6 px-4 py-3 rounded-lg border-2 ${cat?.priority && priorityConfig?.[cat?.priority]?.color}`}
                    >
                      <p className="text-lg font-semibold">
                        {cat?.priority && priorityConfig?.[cat?.priority]?.label}
                      </p>
                      <p className="text-sm">
                        {cat?.priority && priorityConfig?.[cat?.priority]?.description}
                      </p>
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-4 text-lg text-gray-600 mb-6">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      {cat?.priority && cat?.years > 0 &&
                        `${cat?.years} ${cat?.years === 1 ? "año" : "años"}`}
                      {cat?.priority && cat?.years > 0 && cat?.priority && cat?.months > 0 && " y "}
                      {cat?.priority && cat?.months > 0 &&
                        `${cat?.months} ${cat?.months === 1 ? "mes" : "meses"}`}
                  ...
                  <div className="flex items-center gap-2">
                    <span>
                      {cat?.sex === "macho" ? "♂️ Macho" : "♀️ Hembra"}
                    </span>
                  </div>
                  <span className="text-gray-400">•</span>
                  <div className="flex items-center gap-2">
                    <Home className="w-5 h-5 text-[#6A4A8A]" />
                    <span>{cat?.priority && statusConfig?.[cat?.status]?.label}</span>
                  </div>
                </div>
                <div className="mb-6">
